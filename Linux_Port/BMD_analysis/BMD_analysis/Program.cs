...
namespace SuperBMDLib
{
    ...
    {
        //-------------------------------------------------------------------------------
        // AssimpNet のネイティブライブラリ名を最近の glibc 向けに解決する処理
        // AssimpNet 4.1 は dlopen を 'libdl.so' から P/Invoke するが、
        // glibc 2.34 以降（Ubuntu 22.04+ など）には 'libdl.so.2' しか存在しないため読み替える
        //-------------------------------------------------------------------------------
        static void SetupNativeLibraryResolver()
        {
            if (OperatingSystem.IsWindows())
                return;

            System.Runtime.InteropServices.NativeLibrary.SetDllImportResolver(
                typeof(Assimp.AssimpContext).Assembly,
                (libraryName, assembly, searchPath) =>
                {
                    if (libraryName == "libdl.so")
                        return System.Runtime.InteropServices.NativeLibrary.Load("libdl.so.2", assembly, searchPath);
                    return IntPtr.Zero;
                });
        }
        ...
        static void Main(string[] args)
        {
            ...
            // Console.Title は Windows 専用 API のため Linux では設定しない
            if (OperatingSystem.IsWindows())
                Console.Title = "BMD_analysis Console";
            Console.WriteLine("BMD_analysis v2");
            Console.WriteLine();

            //-------------------------------------------------------------------------------
            // BMD ファイルヘッダーに書き込む識別文字列（固定 16 バイト）
            //-------------------------------------------------------------------------------
            String headerString = "BMD_analysisv2.0"; // 16 chars
