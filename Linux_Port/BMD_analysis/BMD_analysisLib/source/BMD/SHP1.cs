                    ...
                    foreach (Primitive prim in pack.Primitives)
                    {
                        ...
                        for (int triIndex = 0; triIndex < triVertices.Count; triIndex += 3)
                        {
                            ...
                            for (int triVertIndex = 0; triVertIndex < 3; triVertIndex++)
                            {
                                ...
                                OpenTK.Mathematics.Vector3 posVec = vertData.Positions[(int)vert.GetAttributeIndex(GXVertexAttribute.Position)];
                                OpenTK.Mathematics.Vector4 openTKVec = new Vector4(posVec.X, posVec.Y, posVec.Z, 1);
                                ...
                                if (vert.VertexWeight.WeightCount == 1)
                                {
                                    if (inverseBindMatrices.Count > vert.VertexWeight.BoneIndices[0])
                                    {
                                        ...
                                        test.Transpose();
                                        Vector4 trans = OpenTK.Mathematics.Vector4.TransformRow(openTKVec, test);
                                        vertVec = new Vector3D(trans.X, trans.Y, trans.Z);
                                    }
                                    else
                                    {
                                        Vector4 trans = OpenTK.Mathematics.Vector4.TransformRow(openTKVec, flatSkeleton[vert.VertexWeight.BoneIndices[0]].TransformationMatrix);
                                        vertVec = new Vector3D(trans.X, trans.Y, trans.Z);
                                    }
                                }
                                ...
                                {
                                    OpenTK.Mathematics.Vector3 nrmVec = vertData.Normals[(int)vert.NormalIndex];
                                    OpenTK.Mathematics.Vector4 openTKNrm = new Vector4(nrmVec.X, nrmVec.Y, nrmVec.Z, 1);
                                    ...
                                    if (vert.VertexWeight.WeightCount == 1)
                                    {
                                        ...
                                        {
                                            Vector4 trans = OpenTK.Mathematics.Vector4.TransformRow(openTKNrm, flatSkeleton[vert.VertexWeight.BoneIndices[0]].TransformationMatrix);
