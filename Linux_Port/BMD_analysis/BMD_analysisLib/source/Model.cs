...
namespace SuperBMDLib
{
    public class Model
    {
        ...
        public Model(Scene scene, Arguments args, List<SuperBMDLib.Materials.Material> mat_presets = null, string additionalTexPath = null)
        {
            ...
            //-------------------------------------------------------------------------------
            // --noskeleton 指定時はボーンデータを除去してスタティックメッシュとして変換する
            // skeleton_root が見つからない FBX (bmd2fbx 出力等) でもクラッシュせずに変換できる
            //-------------------------------------------------------------------------------
            if (args.no_skeleton && root == null)
            {
                Console.WriteLine("[Info] --noskeleton: Stripping bone data, converting as static mesh.");
        ...
        public void ExportAssImp(string fileName, string modelType, ExportSettings settings, Arguments cmdargs)
        {
            ...
            while (!dae.EndOfStream)
            {
                ...
                {
                    // ノード名は属性の出現順に依存しないよう name="..." から正規表現で取得する
                    // （Assimp 4.x は id と name の間に sid を出力することがあるため）
                    string name = System.Text.RegularExpressions.Regex
                        .Match(line, "name=\"([^\"]*)\"").Groups[1].Value;

                    if (Joints.FlatSkeleton.Exists(x => x.Name == name))
                    {
                        // Assimp 4.x 以降は <node> に sid / type 属性を自前で出力するため、
                        // そのまま追記すると属性が重複して不正な XML になる。
                        // 既存の sid / type を取り除いてからジョイント属性を付け直す。
                        string cleaned = System.Text.RegularExpressions.Regex
                            .Replace(line, "\\s+(sid|type)=\"[^\"]*\"", "");
                        string jointLine = cleaned.Replace(">", $" sid=\"{ name }\" type=\"JOINT\">");
