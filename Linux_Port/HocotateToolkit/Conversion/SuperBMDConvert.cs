...
{
    /// <summary>
    /// BMD_analysis / FBX_analysis を内部的に呼び出して BMD/DAE/OBJ/FBX 変換を行うラッパー。
    /// 各 exe は Hocotate_Toolkit と同じ階層の resource/ フォルダに配置してください。
    /// </summary>
    public static class SuperBMDConvert
    {
        private static string? _superBmdExe;

        // Linux版はバイナリ名に拡張子を付けない
        private static string GetSuperBmdExe()
            => _superBmdExe ??= ExeRunner.FindExe("BMD_analysis");
        ...
        /// FBX_analysis を resource/ フォルダに配置する必要があります。
        /// </summary>
        /// <param name="inputBmd">入力 .bmd ファイルパス</param>
        ...
        public static int Bmd2Fbx(string inputBmd, string? outputDir)
        {
            ValidateInput(inputBmd, ".bmd", ".bdl");
            inputBmd = Path.GetFullPath(inputBmd);
            string bmd2fbxExe = ExeRunner.FindExe("FBX_analysis");  // Linux版は拡張子なし
